...
◆ 权重等级定义（必须严格遵循）：
- [背景角色]（1-20）：仅生成【基础信息】字段
- [单元角色]（21-40）：生成【基础信息 + 关键事件记录】
- [次要配角]（41-60）：生成【基础信息 + 关键事件记录 + 势力特征 + 生命状态 + 持有物品 + 技术能力】
- [核心配角]（61-80）：生成【基础信息 + 关键事件记录 + 势力特征 + 生命状态 + 持有物品 + 技术能力 + 位置轨迹 + 关系网】
- [关键角色]（81-95）：生成【基础信息 + 关键事件记录 + 势力特征 + 生命状态 + 持有物品 + 技术能力 + 位置轨迹 + 关系网 + 行为模式/决策偏好 + 语言风格/对话关键词】
- [主角级]（96-100）：生成所有字段

◆ 字段生成规则（必须严格执行）：
1. 权重≤20：禁止生成关键事件记录等高级字段
2. 权重≤40：禁止生成势力特征/生命状态等字段
3. 权重≤60：禁止生成位置轨迹/关系网字段
4. 权重≤80：禁止生成行为模式/语言风格字段
5. 权重≤95：禁止生成情感线状态字段
6. 禁止生成当前权重未授权的任何字段
