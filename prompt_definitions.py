...
- 角色权重：85（关键角色）
位置轨迹：
- 图书馆（时间线：第2章）（事件：调查案件）（同行人物：周老寨主/雷军师/''>）（目的：收集线索）
势力特征：
- 势力归属：
  籍贯/家乡：联邦·蔚蓝星·北境·霜雪岭
...
- 对亲属称谓、官职、别称等非姓名类称呼单独列出，并标注来源或用途

3. **角色名称匹配与更新**
- 若某角色此前已被记录为“李县令”，而在本章首次出现其全名为“李德全”，则：
  - 将该角色的**正式名更新为“李德全”**
  - 将“李县令”归入该角色的**其他称谓**列表下
- 同一角色使用多个称呼时，需判断是否为同一角色，并统一使用数据库中已有的ID及最新正式名

4. **非姓名类角色名称处理**
- 对于以官职、称谓等形式出现的新角色（如“李县令”、“王氏”、“周父”等），仍视为有效角色名称，但需添加**注释说明**
...
1. 输出格式
```
ID0001：张三（别称：无极子；尊称：张真人；官职：玄门长老）
ID0002：李德全（别称：李县令；官职：和田县令）
ID0003：王氏（周明母亲）
ID0004：周父（周明之父）
ID0005：青鳞妖王（尊称：九幽蛇君）
...
请根据上述信息，结合章节内容与已有角色状态，逐个角色进行状态更新。更新需遵循以下规则：
...
|----------|--------------|----------------------------------------------------------------------------|
| 1 - 20   | 背景角色     | 基础信息 + 位置轨迹                                                      |
| 21 - 40  | 单元角色     | 基础信息 + 位置轨迹 + 势力特征                                           |
| 41 - 60  | 次要配角     | 基础信息 + 位置轨迹 + 势力特征 + 关键事件记录                            |
| 61 - 80  | 核心配角     | 基础信息 + 位置轨迹 + 势力特征 + 关键事件记录 + 生命状态 + 持有物品 + 技术能力 + 关系网 |
| 81 - 95  | 关键角色     | 包含上一级字段 + 行为模式/决策偏好 + 语言风格/关键词                     |
| 96 - 100 | 主角级       | 所有字段均需生成                                                         |

> ⚠️ **注意**：禁止生成超出当前角色权重所允许的字段！

---

### ◆ 字段生成规则（必须严格执行）

1. 权重 ≤20：仅生成【基础信息】+【位置轨迹】
2. 权重 ≤40：禁止生成【关键事件记录】及以上字段
3. 权重 ≤60：禁止生成【生命状态】【持有物品】【技术能力】
4. 权重 ≤80：禁止生成【行为模式/决策偏好】【语言风格/关键词】
5. 权重 ≤95：禁止生成【情感线状态】
...
【关系网】（仅当权重≥61时生成）
- 角色名:关系类型,关系强度[低/中/高],互动频率[稀少/普通/频繁]  
*生成数量：权重≥90≤5条，其他≤2条*
...
- 对于非姓名类称呼（如“李县令”、“周父”、“王氏”），保留并添加注释说明。
- 同一角色若多次出现不同称呼，统一使用数据库中已有ID及最新正式名。
- 权重应根据角色在章节中的重要性动态调整（上升或下降），体现角色成长或边缘化趋势。
...
- 禁止使用任何解释、注释、额外文本
- 字段内容为空则留空（不写“无”）
- 禁止使用“编号”两字，直接使用实际ID（如 ID0001：张三）
- 每个字段之间不允许出现空行
- 严格按照字段模板格式输出
...
- 最后出场章节：第7章
位置轨迹：
- 和田县衙（时间线：第7章）（事件：接待访客）（同行人物：衙役甲）（目的：处理政务）
势力特征：
- 势力归属：
  籍贯/家乡：南疆·和田县
  所属势力：地方官府
...
- 最后出场章节：第7章
位置轨迹：
- 宅院内堂（时间线：第7章）（事件：料理家务）（同行人物：婢女）（目的：照顾家人）
势力特征：
- 势力归属：
  籍贯/家乡：南疆·周家村
  所属势力：平民家庭
...
- **对话设计**：  
  ...
  - 每个角色的对话应该符合角色的背景（如没读过书的农民不应该使用复杂词汇，语气应更口语化）
...
当前小说共设计了{Total_volume_number}卷，严格分析上面创作要素与设定，按照以下格式生成第1卷分卷大纲，
{volume_design_format} 

要求：
- 严格遵循创作要素，不要偏离设定
- 当前仅生成第1卷大纲，不要包含后续分卷内容，避免过早出现后续情节，导致后续分卷大纲无法生成
- 世界观需在本卷中初步建立，包含核心规则与设定
- 所有角色需出现在角色发展与互动设计中
- 世界观需通过事件与角色决策自然揭示
- 高潮场景需融合核心主题与世界观特性
- 收尾悬念需为后续卷埋下伏笔
...
当前小说共设计了{Total_volume_number}卷，当前生成第{volume_number}卷分卷大纲，严格按照上面设定，与上一分卷内容，按照如下格式生成第{volume_number}卷《[卷名]》的大纲：
{volume_design_format}
