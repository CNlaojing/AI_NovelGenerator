...
- 情绪演变路径（示例：期待（建立认知）→ 错愕（打破预期）→ 顿悟（重构认知））
- 伏笔编号示例 （示例：编号(类型)：MF001(主线伏笔)、SF001(支线伏笔)、YF001(一般伏笔)、AF001(暗线伏笔)）
- 伏笔状态标注（新埋设/已触发/待回收/回收）
- 如章节无伏笔则留空
- 伏笔需按照编号设置，按照编号标注状态，
...
├─情绪演变：示例：期待（建立认知）→ 错愕（打破预期）→ 顿悟（重构认知）
├─伏笔操作：
│  ├──(编号（类型）+伏笔内容+状态)
│  ├──(编号（类型）+伏笔内容+状态)
│  └──...
├─颠覆指数：Lv.X (3级以上需标注颠覆源)
└─本章简述：（严格≤75字）
...
□ 悬念架构：
  现章伏笔｜{foreshadowing}
  次章预埋｜{next_chapter_foreshadowing}
...
□ 冲突升级：
  ...
  - 在环境描写中预埋{next_chapter_foreshadowing}的暗示
...
1. [冲突类型] 具体段落/设定矛盾点 (关联要素：前文摘要/角色档案/分卷大纲...)
2. [伏笔] 需埋设未埋设或需回收未回收未解决线索 (关联冲突编号#XX)
3. [角色偏移] 人物行为与档案矛盾 (档案记录：XXX)
...
[冲突数量]. [大纲偏离] 与分卷定位不符 (大纲要求：XXX)
