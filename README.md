...
## 🔄 更新日志
- V1.7.4(2026-07-20)
   - **格式稳定性修复**：修复部分模型流式返回时换行分片被过滤，导致章节目录或分卷大纲保存后格式错乱的问题。
   - **目录与分卷校验**：为 `章节目录.txt` 和 `分卷大纲.txt` 增加保存前规范化、格式校验与失败样本保留机制，降低后续流程解析失败概率。
   - **打包配置修正**：修复 `main.spec` 中写死项目路径、缺失 `config.json` 时可能导致打包失败的问题，并同步更新版本号到 `1.7.4`。
...
```
打包完成后，会在 `dist/` 目录下生成对应版本目录（例如 `AI_NovelGenerator V1.7.4`）。

补充说明：
- `main.spec` 已改为自动使用当前项目目录，不再依赖固定盘符路径。
- 即使项目根目录暂时没有 `config.json`，也不会因为该文件缺失而直接导致打包失败；程序首次运行时会自动生成配置文件。
