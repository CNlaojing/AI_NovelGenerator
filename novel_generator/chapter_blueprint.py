...
def analyze_directory_status(filepath: str) -> tuple:
    ...
    try:
        ...
        # 查找所有章节号
        chapter_numbers = re.findall(r'第(\d+)章', content)
        if not chapter_numbers:
            return 0, [], []
            
        last_chapter = max(int(num) for num in chapter_numbers)
...
def get_volume_progress(filepath: str) -> tuple:
    """
    获取当前卷的进度信息
    返回：(当前卷号, 最新章节号, 当前卷起始章节, 当前卷结束章节, 是否在卷尾)
    """
    try:
        # 获取最新章节号
        ...
        if last_chapter == 0:
            ...
            return 1, 0, first_vol['start'], first_vol['end'], False
        else:
            # 超出已定义卷的情况
            last_vol = volume_ranges[-1]
            next_start = last_vol['end'] + 1
            estimated_end = next_start + (last_vol['end'] - last_vol['start'])
            return current_vol, last_chapter, next_start, estimated_end, False
...
def Chapter_blueprint_generate(
    ...
    for vol in target_volumes:
        start_chapter = max(x for x in existing_chapters if x <= vol['end']) + 1 if existing_chapters else vol['start']
